...
class HistoryService {
    ...
    async get() {
        ...
        const response = await fetch(`${Config.getHistory}`, {
            method: 'POST',
            body: data
        });
    ...
    async search(query: string) {
        ...
        const response = await fetch(`${Config.getHistorySearch}`, {
            method: 'POST',
            body: data
        });
        return this._formattedResponse(response);
    ...
    async read(ids: number[]) {
        const data = this._createRequestData({ ids });
        const response = await fetch(`${Config.readHistoryItems}`, {
            method: 'POST',
            body: data
        });
        return this._formattedResponse(response);
